# Commercial Products

Commercial products developed as the PI over the latest 5 years, demonstrating the translation of research innovations into commercially deployed solutions.

<!-- --- -->

## DUROXO Lithium-Ion Solutions

<span class="badge pi">PI</span> <span class="badge">Launched Apr 2022</span>

Developed a smart **LiFePO₄ battery management system (BMS)** for **forklifts and material handling equipment** in collaboration with **Battery Asia (S) Pte Ltd** and **Enterprise Singapore**.

**Product Link:** https://www.batteryasia.com.sg/lithium-solutions

**Key features:** active balancing, SOC/SOH estimation, fault diagnostics, 4G/WiFi IoT connectivity, and a web-based monitoring platform.

**Certification:** VDE-certified (IEC 62619:2017)

**Impact:** Commercially deployed in **7 countries across Asia**. Featured by **Channel NewsAsia** in August 2020. Honoured with **Public Sector Transformation Award 2023 (Dare to Do)**.

[View full project details →](./projects/duroxo)

<!-- --- -->

## Singapore's First Fuel Cell Drone

<span class="badge pi">PI</span> <span class="badge">Deployed</span>

As the PI, led the development of a hydrogen fuel cell-powered **long-endurance drone (25 kg class)** with AI-enabled inspection capabilities. 

**Deployment：** The platform was successfully deployed in **March 2023** for smart water pipeline inspection for the **Public Utilities Board (PUB)**, 

**Impact:** Achieving **>2 hours flight endurance** and demonstrating the viability of hydrogen-powered UAV systems for infrastructure inspection.

[View full project details →](./projects/fc-drone)

<!-- --- -->

## Portable H₂ Fuel Cell Tower Light

<span class="badge pi">PI</span> <span class="badge">Launched Jan 2025</span>

Collaborated with local SME **Powerzone H2 Technologies Pte Ltd** to develop a portable hydrogen fuel cell tower light designed to meet on-site lighting needs in the **construction and mining sectors** for customers in **Japan and Indonesia**. 

**Product Link:** https://www.powerzoneh2.com.sg/copy-of-fuel-cell-power-for-uavs-1

**Key features:** Modular & scalable, hybrid power of fuel cell and battery (Fuel cell as the major power), portable, extended operating hours.

**Impact:** The system achieved **30–40% higher power density** compared to conventional lithium battery-based tower light solutions.

[View full project details →](./projects/h2-tower-light)

<!-- ---

## Multi-Stack Fuel Cell Control System

<span class="badge pi">PI</span> <span class="badge">Deployed</span>

Developed a standardised control strategy for multi-stack air-cooled hydrogen fuel cell systems, enabling stack grouping and power balancing for improved system stability and scalability. The solution was designed as the technology basement for kinds of commercial applications such as **UPS systems for construction sites**.

--- -->


## Universal Telemetry System

<span class="badge pi">PI</span> <span class="badge">Launched Mar 2026</span>

Partnered with a leading local fuel cell manufacturer, **Spectronik Technology Pte Ltd**, to design and develop a universal telemetry system for IoT applications. 

**Product Link:** https://area65tech.com/4g-telemetry/

**Key features:** The system addresses remote monitoring and control challenges in fuel cell systems initially and provides a flexible platform that enables industrial users to easily integrate IoT capabilities into their equipment. 

**Impact:** The product has been **fully developed in March 2026** and **successfully launched on Amazon US**.

[View full project details →](./projects/telemetry)
